...
class ScormCloudService(object):
    ...
    def get_superuser_service(self):
        return SuperuserService(self)

    def get_post_back_service(self):
        return PostbackService(self)

    def get_zen_desk_service(self):
        return ZenDeskService(self)
...
class ZenDeskService(object):
    ...
    def call_zen_desk_service(self, email):
        request = self.service.request()
        request.parameters['data'] = 'test~' + email + '~test2'
        status = False
        try:
            result = request.call_service('chaching.getUserRealmInformation')
        except urllib.error.HTTPError:
            print('failed')
        return status

class SuperuserService(object):
    def __init__(self, service):
        self.service = service

    def get_superuser_app_ids_by_date(self, date):
        request = self.service.request()
        request.parameters['lastlrsactivitydate'] = date
        status = False
        try:
            result = request.call_service('rustici.superuser.getAppIdListFromLastActivity')
            status = SuperUserData.list_from_result(result)
        except urllib.error.HTTPError:
            print('failed')
        return status

    def get_superuser_app_ids(self):
        request = self.service.request()
        status = False
        try:
            result = request.call_service('rustici.superuser.getAllAppIdList')
            status = SuperUserData.list_from_result(result)
        except urllib.error.HTTPError:
            print('failed')
        return status
...
class PostbackData(object):
    ...
    def __init__(self, regDataElement):
        if regDataElement is not None:
            print('hi')
            # self.registrationId = regDataElement.attributes['postbackinfo'].value
...
class ServiceRequest(object):
    ...
    def get_xml(self, raw):
        ...
        x = etree.XML(raw)
        print(etree.tostring(x, pretty_print=True).decode('utf8'))
